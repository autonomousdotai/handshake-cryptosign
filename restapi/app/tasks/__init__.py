...
def update_feed(handshake_id):
	try:
		...
		hs = {
			...
			"outcome_total_amount_s": '{}'.format(outcome.total_amount if outcome.total_amount is not None else 0),
			"outcome_total_dispute_amount_s": '{}'.format(outcome.total_dispute_amount if outcome.total_dispute_amount is not None else 0)
		}
